...
export const updateRemoteData = (product,i) => (dispatch) => {
  ...
  if (product.inventoryCount) {
    console.log("product.inventoryCount before add << ", product.inventoryCount," id: ",i);
    return superagent
      .get(`${url}/${product._id}`)
      .then((DBproduct) =>
      // console.log(' inventoryCount before put ===> ',DBproduct.body[0].inventoryCount)
        ...
        .put(`${url}/${DBproduct.body[0]._id}`)
          .send({ ...DBproduct, inventoryCount: DBproduct.body[0].inventoryCount - 1 })
      )
      .then((data) => {
        // call my action after getting the API response.
        console.log("product.inventoryCount after add >>", data.body.inventoryCount," id: ",i);
